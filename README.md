...
| **Chat simulator** | A separate Chat screen where a real Claude model (your Anthropic API key, set in Settings) acts as the host: it sees the server's tools, decides when to call them, and results render as widgets inline in the transcript — a local ChatGPT-developer-mode preview |
